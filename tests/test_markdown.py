...
class TestMarkdown(unittest.TestCase):
    ...
    # Pre-compile regular expressions to prevent redundant parsing and compilation overhead
    _SLUG_RE_NON_ALNUM = re.compile(r'[^a-z0-9\s-]')
    _SLUG_RE_SPACES = re.compile(r'\s+')
    _SLUG_RE_DASHES = re.compile(r'-+')
    ...
    def slugify(self, text):
        ...
        slug = text.lower()
        slug = self._SLUG_RE_NON_ALNUM.sub('', slug)
        slug = self._SLUG_RE_SPACES.sub('-', slug)
        slug = self._SLUG_RE_DASHES.sub('-', slug)
        return slug.strip('-')
