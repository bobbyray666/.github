...
class TestMarkdown(unittest.TestCase):
    ...
    SLUG_SPACES_RE = re.compile(r'\s+')
    SLUG_HYPHENS_RE = re.compile(r'-+')
    ...
    def slugify(self, text):
        ...
        slug = self.SLUG_SPACES_RE.sub('-', slug)
        slug = self.SLUG_HYPHENS_RE.sub('-', slug)
        return slug.strip('-')
        ...
        for filepath in self.get_markdown_files():
            with open(filepath, "r", encoding="utf-8") as f:
                ...
                for idx, line in enumerate(f, 1):
                    ...
                    match = self.LIST_RE.match(stripped)
                    if match:
