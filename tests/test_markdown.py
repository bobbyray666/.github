...
class TestMarkdown(unittest.TestCase):
    ...
    SLUG_NON_ALNUM_RE = re.compile(r'[^a-z0-9\s-]')
    SLUG_WHITESPACE_RE = re.compile(r'\s+')
    SLUG_MULTI_HYPHEN_RE = re.compile(r'-+')
    HEADING_FORMAT_RE = re.compile(r'^(#+)([^#\s].*)$')
    LIST_ITEM_RE = re.compile(r'^(\s*)([-*+]|\d+\.)([^\s].*)$')
    HEADING_EXTRACT_RE = re.compile(r'^(#+)\s+(.+)$')
    ...
    def slugify(self, text):
        ...
        slug = text.lower()
        slug = self.SLUG_NON_ALNUM_RE.sub('', slug)
        slug = self.SLUG_WHITESPACE_RE.sub('-', slug)
        slug = self.SLUG_MULTI_HYPHEN_RE.sub('-', slug)
        return slug.strip('-')
        ...
        for filepath in self.get_markdown_files():
            with open(filepath, "r", encoding="utf-8") as f:
                ...
                for idx, line in enumerate(f, 1):
                    ...
                    match = self.LIST_ITEM_RE.match(stripped)
                    if match:
    ...
    def _extract_heading_slugs(self, lines):
        ...
        headings = []
        for line in lines:
            match = self.HEADING_EXTRACT_RE.match(line.strip())
            if match:
    ...
    def test_markdown_links(self):
        ...
        for filepath in self.get_markdown_files():
            ...
            matches = self.LINK_RE.findall(content)
