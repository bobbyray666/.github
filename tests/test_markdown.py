...
class TestMarkdown(unittest.TestCase):
    ...
    RE_SLUGIFY_NON_ALNUM = re.compile(r'[^a-z0-9\s-]')
    RE_SLUGIFY_SPACE = re.compile(r'\s+')
    RE_SLUGIFY_HYPHEN = re.compile(r'-+')
    RE_HEADING_FORMAT = re.compile(r'^(#+)([^#\s].*)$')
    RE_LIST_FORMAT = re.compile(r'^(\s*)([-*+]|\d+\.)([^\s].*)$')
    RE_HEADING_FINDER = re.compile(r'^(#+)\s+(.+)$')
    ...
    def slugify(self, text):
        ...
        slug = text.lower()
        slug = self.RE_SLUGIFY_NON_ALNUM.sub('', slug)
        slug = self.RE_SLUGIFY_SPACE.sub('-', slug)
        slug = self.RE_SLUGIFY_HYPHEN.sub('-', slug)
        return slug.strip('-')
        ...
        for filepath in self.get_markdown_files():
            with open(filepath, "r", encoding="utf-8") as f:
                ...
                for idx, line in enumerate(f, 1):
                    ...
                    match = self.RE_HEADING_FORMAT.match(stripped)
                    self.assertIsNone(
                        match,
        ...
        for filepath in self.get_markdown_files():
            with open(filepath, "r", encoding="utf-8") as f:
                ...
                for idx, line in enumerate(f, 1):
                    ...
                    match = self.RE_LIST_FORMAT.match(stripped)
                    if match:
    ...
    def _extract_heading_slugs(self, lines):
        ...
        headings = []
        for line in lines:
            match = self.RE_HEADING_FINDER.match(line.strip())
            if match:
    ...
    def test_markdown_links(self):
        ...
        for filepath in self.get_markdown_files():
            ...
            matches = self.RE_LINK.findall(content)
